...
def slugify_filename(instance, filename):
    ...
    clean_filename = [slugify(split_filename[0]).replace('-','_'), split_filename[-1]]
    filename = '.'.join(clean_filename)
    return filename.format(instance, filename)
...
def attempt_protected_media_value_conversion(request, value):
    try:
        if value.startswith(page_choices['PROTECTED_MEDIA_URL']):
...
def gen_cache_prefix(self, request):
    seg_str = None
    try:
        user_segs = []
...
class LocalPage(collections.abc.Sequence):
